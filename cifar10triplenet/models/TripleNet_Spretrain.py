...
class TripleNet(nn.Module):
    def __init__(self, pretrained=True, weight_path=''):
        ...
        if pretrained:
          weight_file = '%smodel_triplenet_s_100.pth'%(weight_path)            
          if not os.path.isfile(weight_file):
            print(weight_file,'is not found')
            exit(0)
          weights = torch.load(weight_file, map_location=torch.device('cuda'))
...
def triplenet_s():
    return TripleNet(pretrained=True, weight_path='')