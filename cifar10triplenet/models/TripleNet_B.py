...
class TripleNet(nn.Module):
    def __init__(self, pretrained=True, weight_path=''):
        super().__init__()
        first_ch  = [32, 64]
        second_kernel = 3
        grmul = 1.7
        ...
        self.base.append ( DenseLayer(256, 32, kernel=3, stride=1,  bias=False) ) 
        self.base.append ( DenseLayer(288, 32, kernel=3, stride=1,  bias=False) )         
        self.base.append ( ConvLayer(320, ch_list[0], kernel=1) )
        ch = ch_list[0]
        if downSamp[0] == 1:
        ...
        # Fifth Block     
        self.base.append ( Bottleneck(ch, 240))
        self.base.append ( Bottleneck(720, 360))
        ...
        if pretrained:
          weight_file = '%s\\triple_b.pth'%(weight_path)            
...
def triplenet_b():
    return TripleNet(pretrained=False, weight_path='D:TripleNet\\pre-trained-cifar10\\')